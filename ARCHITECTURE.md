...
Public webpage text and multimodal YouTube evidence enter through the same document boundary. For videos, bounded low-resolution download and FFmpeg frame sampling feed mixed image/text inputs to the configured OpenAI vision model, then its timestamped observations are merged with captions and metadata. URL validation rejects credentials, local hosts, non-public IP addresses, unsafe redirects, oversized responses, and unsupported content types; extracted content is stored with source provenance and explicitly marked as untrusted before normal chunking, retrieval, citation, and debate processing.
